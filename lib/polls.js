if (!window.notcrap) {
    ...
    e.src = "https://bru02.000webhostapp.com/lib/notcrap.js", document.head.appendChild(e);
}

function setUpStorage() {
     if(null != typeof Storage) {
         if((!localStorage.polls)||localStorage.polls=="undefined"||localStorage.polls=="null") localStorage.polls = "[]"
     return JSON.parse(localStorage.polls);
     } else return false;
}
...
function vote(sel) {
    ...
    t.getAttribute("vote") && ajax({
        ...
        let t = JSON.parse(e), i = [], o = 0, a = r.querySelectorAll(".vote-opt");
        if (t.forEach(e => {
            i.push(e[1]);
        }), (t = (e => {
            return percentile(percentile(i));
            function percentile(e) {
                if (!Array.isArray(e)) return "err";
                var t = e, r = [], i = (e => {
                    let t = 0;
                    return e.forEach(e => {
                        t += e;
                ...
                for (let e = 0, o = t.length; o > e; e++) t[e] = "Nan" == +t[e] ? 0 : t[e], r[e] = Math.round(100 / i * t[e]);
                return r;
            }
        })()).forEach(e => {
            let t = e;
            a[o].querySelector(".bar").setAttribute("value", t), a[o].querySelector(".bar").show(), 
            (() => {
                for (let e = 0, t = document.querySelectorAll("quiz[id] div"), r = t.length; r > e; e++) move(t[e].children[0], t[e].getAttribute("value"));
                function move(e, t) {
                    var r = 1, i = setInterval(() => {
            ...
            let arr = setUpStorage();
            arr.push([ r.getAttribute("id"), v ]), localStorage.polls = JSON.stringify(arr);
        }
    });
}
...
window.addEventListener("load", () => {
    ...
    for (let e = 0, i = t.length; i > e; e++) {
        ...
        ajax({
            ...
            let data = JSON.parse(e), title = data.title;
            pub = data.canvote=="0"?0:1;
            i.innerHTML = title;
        });
        ...
        ajax({
            ...
            let da = JSON.parse(el), r = 0;
            da.forEach(ek => {
                let d = ek[0], i = generateId(20);
            ...
            if (setUpStorage() || 0 === pub) {
                let arr = setUpStorage();
                let index = arr.indexOf(t[e].getAttribute("id"));
                if(0 === pub) {
                    vote(t[e].querySelectorAll(".vote-opt")[0].firstChild);
                t[e].querySelectorAll(".vote-btn").forEach((e)=>{
                    e.hide();
                    e.nextSibling.addClass("no");
                    e.checked = false;
                });
                if(index==-1) {
                arr.forEach((n,i) => {
                    if(n[0] == t[e].getAttribute("id")) arr.splice(i, 1);
                }),
                localStorage.polls = JSON.stringify(arr);
                }
                }else{
                arr.forEach(n => {
                    if(n[0] == t[e].getAttribute("id")) vote(t[e].querySelectorAll(".vote-opt")[0].firstChild);
                }),
                localStorage.polls = JSON.stringify(arr);
                }
            }
