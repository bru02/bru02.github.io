...
(function (root, factory) {
	...
	const ajax = (data, readyfunc) => {
		...
		(xhttp = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP")).open(type, data.url, async),
		xhttp.onreadystatechange = (() => {
			xhttp.readyState > 3 && 200 == xhttp.status && f(xhttp.responseText);
		}),
		"POST" == type && (dataString = "string" == typeof str ? str : Object.keys(str).map(r => `${encodeURIComponent(r)}=${encodeURIComponent(str[r])}`).join("&"),
